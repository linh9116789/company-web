                     ...
                     <table class="table table-hover table-condensed">
                        ...
                        <tbody>
                           ...
                           <form action='<?php echo BASE_URL ?>/giohang/updategiohang' method="post">
                              <?php                         
                                 foreach($_SESSION['shopping_cart'] as $key =>$pro_cart){
                                    $totalpro = $pro_cart['pro_price'] * $pro_cart['pro_quantity'];
                                    $total +=$totalpro;      
                              ?>
                              <tr class="tr">
                                 <td data-th="Hình ảnh">
                                    <div class="col_table_image col_table_hidden-xs"><img src="<?php echo BASE_URL ?>/public/uploads/product/<?php echo $pro_cart['pro_avatar'] ?>" class="img-responsive"/></div>
                                 </td>
                                 <td data-th="Sản phẩm">
                                    <div class="col_table_name">
                                       <h4 class="nomargin"><?php echo $pro_cart['pro_name'] ?></h4>
                                    </div>
                                 </td>
                                 <td data-th="Giá"><span class="color_red font_money"><?php echo number_format($pro_cart['pro_price'],0,',','.') ?></span></td>
                                 <td data-th="Số lượng">
                                    <div class="clear margintop5">
                                       <div class="floatleft"><input type="number" class="inputsoluong" min='1' name="pro_quantity[<?php echo $pro_cart['pro_id'] ?>]" value="<?php echo $pro_cart['pro_quantity'] ?>"></div>
                                       <input type="hidden" name="check" value="999">
                                    </div>
                                    <div class="clear"></div>
                                 </td>
                                 <td data-th="Thành tiền" class="text_center"><span class="color_red font_money"><?php echo number_format($totalpro,0,',','.') ?> đ</span></td>
                                 <td class="actions aligncenter">
                                    <button type="submit" name="delete_cart" style="box-shadow:none;" value="<?php echo $pro_cart['pro_id'] ?>" class="btn btn-danger btn-sm">Xóa</button>
                                    <button type="submit" name="updata_cart" style="box-shadow:none;margin-top:2px" value="<?php echo $pro_cart['pro_id'] ?>" class="btn btn-primary btn-sm" >Cập nhật</button>  
                                 </td>
                              </tr>
                              <?php  
                              }
